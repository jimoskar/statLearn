...
plot(cv.diabetes$size, 
  cv.diabetes$dev, type = "b", xlab = "Terminal Nodes", ylab = "Misclassifications")
...
The misclassification error for the pruned tree is reported above. It is apparent that the misclassification error on the test set is slightly lower for the pruned tree with 3 terminal nodes (the predicions are split only in half, since the left subtree predicts 0 in both cases), compared to the unpruned tree. Despite this, the errors are very similar, which suggests that a more advanced method should be tested. 
