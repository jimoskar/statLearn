...
biplot(pr.out, scale = 0, cex = 0.2)
# Rotations er loadings (som er koeffisientene til PCene).
plot(pr.out$rotation[, "PC1"], pr.out$rotation[, "PC2"]) # Usikker på om det er dette de ønsker!

# x er scores (som er verdiene til PCene). Tror det er dette de mener!
plot(pr.out$x[, "PC1"], pr.out$x[, "PC2"])
text(pr.out$x[,"PC1"],pr.out$x[, "PC2"],rownames(GeneData),cex=0.6) # Eller dette de mener? Vil vel farge alle til venstre noe, og alle til høyre noe annet? 
```
...
\textcolor{red}{Skjønner ikke dette :(}
