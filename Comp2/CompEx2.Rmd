...
\textcolor{green}{grunnen til at dn er kvadrert er vel for at størrelsen ikke skal bli negativ? Men sikkert greit å spørre!} \textcolor{red}{Ja, enig i det, men jeg tenkte ikke spesifikt på om den er kvadrert eller ikke. Det jeg lurer på er typ: Er den FALSE fordi den penalizer integralet til den dobbeltderiverte, og ikke dobbeltintegralet i seg selv. Man kan jo kanskje argumentere for at den netto penalizer kun den enkelderiverte, pga integralet? Vanskelig å si synes jeg :)}
...
(ii) \textcolor{red}{SPM: Hvorfor bruke random forest her, og ikke bagging eller boosting?} \textcolor{blue}{Hun sa at RF var mest relevant da vi kun diskuterte dette i forbindelse med klassifisering.}
For the more advanced method, a random forest approach is considered. The number of trees is chosen to be $B = 1000$, since this amount seems to be sufficient for the test error to settle. Furthermore, since $p = 7$, we choose the tuning parameter $m = 3 \approx \sqrt{p}$, because this is recommended in ISLR. 
...
\textcolor{red}{Tror ikke det er meningen å få samme svar for begge gamma :( må spørre om det!)}
...
# Bare for å teste: Det samme gjør kmeans. Kanskje det er meningen ellerno?
# Må noen kolonner tas bort ellerno?
km.out <- kmeans(GeneData, 2, nstart = 20)
km.out$cluster
```
 \textcolor{red}{Every method classifies everything correctly!!?} \textcolor{green}{Ser sånn ut ja! Jeg aner ikke hvorfor dette skulle skje!?}
...
Since the first principal component has the highest variability in the data, it can be used to find which genes that vary the most across the two groups. The genes with the highest loadings (rotations) are the ones that contribute the most to the variability. Hence, using only the first principal component, yields

\textcolor{green}{formulerte det litt annerledes: }
From the biplot \textcolor{red}{usikker på om det er et biplot? (tenker på selve terminologien)} in e), we can see that the two groups are separated along the axis of the first principal component (PC1) Hence, to determine which genes vary the most across the two groups, we examine the loadings of the first principle component \textcolor{red}{Enig i det du skriver, men dette kan man også påstå uten plottet tenker jeg, siden PC1 per def har mest variabilitet i dataen. } \textcolor{green}{Tror vi trenger plottet for å påstå det faktisk. Vi kan jo tenke oss en situasjon der vi ser at PC2 separerer gruppene?}

```{r}
sort(abs(pr.out$rotation[, "PC1"]), decreasing = T)[1:10]
```
\textcolor{red}{Som et annet forslag: Burde kanskje i stedet bruke alle PC-ene til å finne de som varierer mest? Dvs. på akkurat samme måte som her, bare at man kumulativt over alle PC-ene finner hvilke loadings som er størst?! Tror det vil gi mer nøyaktige svar. I det minste interessant å sammenligne resultatene fra det, men det som er ovenfor!} \textcolor{green}{jeg tror det er riktig det du har gjort :) Tror ikke det er noe vits i se på andre PC enn PC1, da denne separerer gruppene helt. La inn et alternativt forlsag til tekst over, hva tenker du?} \textcolor{red}{Tenker det samme som deg, men hadde vært interessant å se om det hadde endret på noe.}
