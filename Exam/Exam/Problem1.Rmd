...
```
The test MSE for the full multiple linear regression is reported above. We see that lasso yields a slightly lower test MSE, which could indicate that not all variables are as relevant and only contribute to a larger variance in the fit. From the summary, we also see that not all covariates are significant with respect to the p-value. We see that lasso only selects age, height, abdomen and wrists. 
