...
\end{split}
$$
FOR NOE GRISERI, UMULIG Å FORENKLE? SE PÅ DETTE SENERE. 
...
Comments: The variance begins at `r variance(0, X, x0, sigma)` and decreases with $\lambda$. Hence, it is apparent that the ridge regression estimator is advantageous compared to the OLS estimator, since the variance is decreasing with $\lambda$. In the end, when adding the changes in bias and variance, it is at this stage possible for the ridge regression estimator to have a lower MSE than the OLS estimator. However, we do not know if this is the case yet. 
...
Comments: Now we are able to conclude that the ridge regression estimator has a lower MSE compared to the OLS estimator, as gathered from the blue line in the plot. The optimal value of $\lambda$, which minimizes MSE, is `r lambdas[which.min(exp_mse)]`. Hence, despite the fact that the bias is higher for the ridge regression estimator, the total expected MSE is lower, since the decrease in variance counters the increase in bias. 
...
<<<<<<< HEAD
=======
knitr::kable(table(Deceased = d.corona$deceased))
>>>>>>> e5d30fa88c2ed1391adcd779c72446ba3b42cfce

table1 <- summarise(d.corona, deceased = sum(d.corona$deceased),
                   non_deceased = nrow(d.corona) - deceased)
knitr::kable(table1) # Prøvde å sette kolonne-navn, men fikk det ikke til.
...
knitr::kable(table4) # Må hente ut øverste raden herfra, men klarte det ikke. 
=======
knitr::kable(table(d.corona$country, d.corona$deceased))
>>>>>>> e5d30fa88c2ed1391adcd779c72446ba3b42cfce
```
...
predict(lm.fit, newdata = x0, type = "response")
```
...
```{r}
log.fit1 <- glm(deceased ~., data = d.corona, family=binomial)
summary(log.fit1)
```

(i) As seen above, the coefficient for male is positive (`r coefficients(log.fit1)[2]`), which means that a male has higher risk than a female. This is because the value of this coefficient is added on top of the interceipt, which corresponds to an individual that is in the reference level, i.e. a female. 

```{r}
log.fit2 <- glm(deceased ~ sex + age*country, data = d.corona, family=binomial)
summary(log.fit2)
```

(ii) As seen above, we have fitted a logistic regression with an interaction term between `age` and `country`, in addition to the rest of the terms, to see how the model predicts the risk factors of age and countries combined. Since `France` is the reference level, and the interaction-term coefficient `age:countryindonesia` has the value `r coefficients(log.fit2)[7]`, we can conclude that age is not a greater risk factor for the French population than for the Indonesian population. This is because the value of the coefficient is added on top of the reference level for the `age:contry`-set of coefficients, which, as already noted, is `France`.

NOTE: I am not sure whether or not we should use logistic or linear regression for these Q's!!!
